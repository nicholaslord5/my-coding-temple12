...
# Task 1: Formatting Flight Itineraries Create a Python function that takes a list of tuples as an 
# argument. Each tuple contains information about a flight itinerary: (traveler_name, origin, destination). 
...
def format_itineraries(itineraries):
    result = ""
    for t, (traveler_name, origin, destination) in enumerate(itineraries, start=1):
        result += f"Itinerary {t}: {traveler_name} - From {origin} to {destination}\n"
    return result
...
list = format_itineraries(itineraries)
print(list)
